...
public class JoshText extends JComponent implements Scrollable,ComponentListener,ClipboardOwner,
	...
	public JoshText(String[] lines)
		...
		fc.setFileFilter(new CustomFileFilter(Messages.getString("JoshText.TYPE_TXT"),".txt")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	*/
	public void Paste()
		{
		UndoPatch up = new UndoPatch(Math.min(caret.row,sel.row),Math.max(Math.max(caret.row,sel.row),
