...
public class GmStreamEncoder
	...
	public void writeTree(DefaultMutableTreeNode group,Gm6File src) throws IOException
		{
		if (group.isLeaf())
			{
			writei(3);
			ResNode leaf = (ResNode) group;
			writei(leaf.kind);
			if (leaf.resourceId != null)
				{
				writei(leaf.resourceId.value);
				}
			else
				{
				writei(0);
			...
			switch (leaf.kind)
				{
				case Resource.SPRITE:
					writeStr(src.getSprite(leaf.resourceId).Name);
					break;
				case Resource.SOUND:
					writeStr(src.getSound(leaf.resourceId).Name);
					break;
				case Resource.BACKGROUND:
					writeStr(src.getBackground(leaf.resourceId).Name);
					break;
				case Resource.PATH:
					writeStr(src.getPath(leaf.resourceId).Name);
					break;
				case Resource.SCRIPT:
					writeStr(src.getScript(leaf.resourceId).Name);
					break;
				case Resource.FONT:
					writeStr(src.getFont(leaf.resourceId).Name);
					break;
				case Resource.TIMELINE:
					writeStr(src.getTimeline(leaf.resourceId).Name);
					break;
				case Resource.GMOBJECT:
					writeStr(src.getGmObject(leaf.resourceId).Name);
					break;
				case Resource.ROOM:
					writeStr(src.getRoom(leaf.resourceId).Name);
					break;
				default:
					writeStr((String) leaf.getUserObject());
					break;
				}
			writei(0);
			}
			...
			{
			ResNode branch = (ResNode) group;
			writei(branch.status);
			writei(branch.kind);
			writei(0);
			writeStr((String) branch.getUserObject());
			writei(branch.getChildCount());
			for (int i = 0; i < branch.getChildCount(); i++)
				{
				writeTree((DefaultMutableTreeNode) branch.getChildAt(i),src);
				}//children
			}//isLeaf
		}//writeTree
	}//class