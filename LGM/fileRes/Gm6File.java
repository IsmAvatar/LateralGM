...
public class Gm6File
	...
	private void getResUnsafe(ResNode node)
		{
		switch (node.kind)
			{
			case Resource.SPRITE: node.resourceId = getSpriteUnsafe(node.id).Id; break;
			case Resource.SOUND: node.resourceId = getSoundUnsafe(node.id).Id; break;
			case Resource.BACKGROUND: node.resourceId = getBackgroundUnsafe(node.id).Id; break;
			case Resource.PATH: node.resourceId = getPathUnsafe(node.id).Id; break;
			case Resource.SCRIPT: node.resourceId = getScriptUnsafe(node.id).Id; break;
			case Resource.FONT: node.resourceId = getFontUnsafe(node.id).Id; break;
			case Resource.TIMELINE: node.resourceId = getTimelineUnsafe(node.id).Id; break;
			case Resource.GMOBJECT: node.resourceId = getGmObjectUnsafe(node.id).Id; break;
			case Resource.ROOM: node.resourceId = getRoomUnsafe(node.id).Id; break;
	...
	public void LoadGm6File(String FileName,ResNode root) throws Gm6FormatException
		...
		try
			...
			Stack<ResNode> nodes = in.readContents(root);
			while(!nodes.isEmpty())
				getResUnsafe(nodes.pop());
			clock.stop();
	...
	public void WriteGm6File(String FileName,ResNode root)
		...
		try
			...
			for (int i = 0; i < 11; i++)
				{
				out.writeTree((DefaultMutableTreeNode) root.getChildAt(i),this);
				}
			out.close();
			}
		catch (FileNotFoundException ex)
