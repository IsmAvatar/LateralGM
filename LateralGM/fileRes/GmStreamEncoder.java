...
public class GmStreamEncoder
	...
	public void writeTree(ResNode node,Gm6File src) throws IOException
		...
		switch (node.kind)
			{
			case Resource.SPRITE:
				writeStr(src.getSprite(node.resourceId).Name);
				break;
			case Resource.SOUND:
				writeStr(src.getSound(node.resourceId).Name);
				break;
			case Resource.BACKGROUND:
				writeStr(src.getBackground(node.resourceId).Name);
				break;
			case Resource.PATH:
				writeStr(src.getPath(node.resourceId).Name);
				break;
			case Resource.SCRIPT:
				writeStr(src.getScript(node.resourceId).Name);
				break;
			case Resource.FONT:
				writeStr(src.getFont(node.resourceId).Name);
				break;
			case Resource.TIMELINE:
				writeStr(src.getTimeline(node.resourceId).Name);
				break;
			case Resource.GMOBJECT:
				writeStr(src.getGmObject(node.resourceId).Name);
				break;
			case Resource.ROOM:
				writeStr(src.getRoom(node.resourceId).Name);
				break;
			default:
				writeStr((String) node.getUserObject());
				break;
		...
		for (int i = 0; i < node.getChildCount(); i++)
			writeTree((ResNode) node.getChildAt(i),src);
		}//writeTree
	}//class